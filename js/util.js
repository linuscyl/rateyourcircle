...
class Finder extends Solver {
  ...
  get score() {
    ...
    const DGAP =
      math.max(maxGap, 2 * math.PI - (angles[angles.length - 1] - angles[0])) /
      (2 * math.PI);
...
class App {
  constructor(
    ...
    // finding clear canvas button node
    this.clearCanvasButton = document.getElementById(clearCanvasButtonID);
  ...
  resizeCanvas() {
    ...
    let save = this.ctx.getImageData(
      0,
      0,
  ...
  addListeners() {
    ...
    this.handleOnclickResetButton = this.handleOnclickResetButton.bind(this);

    // handle mouse down
    this.canvasContainer.addEventListener("mousedown", this.startPath);
    this.canvasContainer.addEventListener("touchstart", this.startPath);

    // handle mouse move
    this.canvasContainer.addEventListener("mousemove", this.continuePath);
    this.canvasContainer.addEventListener("touchmove", this.continuePath);

    // handle mouse up
    this.canvasContainer.addEventListener("mouseup", this.endPath);
    this.canvasContainer.addEventListener("touchend", this.endPath);
    ...
    this.evaluateButton.addEventListener("click", this.findCricle);

    // handle click on clear canvas button
    this.clearCanvasButton.addEventListener(
      "click",
      this.handleOnclickResetButton
    );

    // handle info press
    this.info.addEventListener("click", this.infoHandler);
  }
}
